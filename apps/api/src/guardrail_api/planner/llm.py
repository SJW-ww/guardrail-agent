...
class LLMPlanner:
    """一步规划:一句意图 → 一条提议。多步编排留给后续版本,先把一步做扎实。"""
    ...
    async def draft(self, session: Any, intent: str, *, actor: str) -> Proposal:
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            try:
                proposal = self._validate(raw, order=order)
            ...
            proposal.planner = "llm"
            # 定档与审批交给策略引擎,模型填的 risk_level / requires_approval 一律作废
            proposal = finalize(
                proposal, actor=actor, registry=self.registry, settings=self.settings
            )
            logger.info(
                "LLM 提议通过校验:action=%s confidence=%.2f(第 %d 次尝试)",
                proposal.action,
                proposal.confidence,
                attempt,
            )
        ...
        raise ProposalRejected(
            f"模型连续 {max_attempts} 次没能给出合规提议,最后一次错误:{last}",
            attempts=attempts,
    ...
    def _validate(self, raw: str, *, order: Order) -> Proposal:
        try:
        ...
        try:
            ...
            raise ProposalRejected(f"提议字段不合规:{_describe_validation_error(exc)}") from exc

        if proposal.action not in self.registry:
            raise ProposalRejected(
                f"工具 {proposal.action} 不存在,可用工具:{self.registry.names()}"
            )
        ...
        if target_id is not None and int(target_id) != order.id:
            raise ProposalRejected(
                f"提议里的 order_id={target_id} 与已解析的订单 {order.id}({order.order_no})不一致"
            )
        target_no = arguments.get("order_no")
        if target_no is not None:
            if str(target_no) != order.order_no:
                raise ProposalRejected(
                    f"提议里的 order_no={target_no} 与已解析的订单 {order.order_no} 不一致"
                )
        ...
        try:
            self.registry.validate_args(proposal.action, proposal.arguments)
        except ToolArgumentError as exc:
            raise ProposalRejected(f"工具入参不合规:{exc.message}") from exc

        # 这里只做「能不能执行」的校验;风险等级与审批要求留给 finalize 按策略引擎填。
        return proposal
