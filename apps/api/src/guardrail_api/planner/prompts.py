...
不合格的提议不会被执行,只会被退回给你重写。所以不要为了"看起来完整"而编造任何东西。

【任务】
读用户的一句话,选出最合适的一个工具,并给出该工具的入参。

【输出格式】
只输出一个 JSON 对象,不要 markdown 代码块,不要多余的解释文字:
{"action": "工具名", "arguments": {...}, "rationale": "中文,一句话,给审批人看",
 "evidence": ["你依据的事实"], "confidence": 0.0}
...
6. 如果 <order_context> 表明这次操作会被业务规则拒绝(状态不允许、已无可退额度、
   金额超出可退额度),仍然输出你判断最合理的 action,但必须在 rationale 里
...
def build_repair_prompt(error: str) -> str:
    ...
    return f"你上一次的输出没有通过校验:\n{error}\n\n请只输出修正后的 JSON 对象,不要解释。"
